  ...
  *)
  ...
  let malloc_with_gc s m e c k =
    if List.length m < mem_limit then
      ...
      (* hint : Add the code that marks the reachable locations.
       * let _ = ... 
       *)
	  let rec chk_loc l = 
		if List.mem l !reachable_locs then ()
		else let _ = reachable_locs := l :: !reachable_locs in chk_value (load l m)
	  and chk_rec r = 
	    let chk_rec_element (id,l) = chk_loc l in
		List.iter chk_rec_element r
	  and chk_value v =
	    match v with
		| L l -> chk_loc l
		| R r -> chk_rec r
	  ...
	  and chk_cmd cmd = 
	    match cmd with
		| PUSH o -> chk_obj o
		| JTR (c1, c2) -> let _ = chk_command c1 in chk_command c2
		| _ -> ()
	  and chk_obj o = 
		match o with
		| Val v -> chk_value v
		| Fn (_,c) -> chk_command c
		| _ -> ()
	  and chk_command c = List.iter chk_cmd c
	  and chk_proc (x,c,e) = let _ = chk_command c in chk_env e
	  and chk_evalue ev = 
	    match ev with
		| Loc l -> chk_loc l
		| Proc p -> chk_proc p
	  and chk_env e =
	    let chk_env_element (id,ev) = chk_evalue ev in
		List.iter chk_env_element e
	  and chk_svalue sv = 
	    match sv with
		| V v -> chk_value v
		| P p -> chk_proc p
		| M (id,ev) -> chk_evalue ev
	  and chk_stack s = List.iter chk_svalue s
	  and chk_memory m = 
	    let chk_memory_element ((b,o),v) = 
		  try
		    let _ = List.assoc b !reachable_locs in
			chk_loc (b,o)
	      with Not_found -> ()
	    in
		List.iter chk_memory_element m
	  and chk_cont k = 
	    let chk_cont_element (c,e) = let _ = chk_command c in chk_env e in
		List.iter chk_cont_element k
	  in

	  let _ = chk_stack s in
	  let _ = chk_env e in
	  let _ = chk_command c in
	  let _ = chk_cont k in
	  let _ = chk_memory m in
