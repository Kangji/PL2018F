...
struct
  ...
  let op2fn =
    ...
    | EQ -> (fun (v1,v2) ->
			...
			| _ -> raise (TypeError "invalid type comparison")
			)
	...
	| LET (e1, e2) ->
	  (match e1 with
	   ...
	   | REC (f, x, e) ->
	     let e1 = FN (x, e) in
		 let (v1, m') = eval env mem e1 in
		 let (c, e') = getClosure v1 in
		 (match c with
		  | Fun (loc_i, loc_e) -> eval (env @+ (f, Closure (RecFun (f, loc_i, loc_e), e'))) m' e2
		  | _ -> raise (RunError "recursion error")
		 )
	  )
    | IF (e1, e2, e3) ->
      let (v1, m') = eval env mem e1 in
